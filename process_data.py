...
#用于对训练数据集与微调数据集进行处理（选择性搜索、数据存取等）
...
class Train_Alexnet_Data(object):
    ...
    '''
    def __init__(self):
        self.train_batch_size = cfg.T_batch_size#64
        self.image_size = cfg.Image_size#224

        self.train_list = cfg.Train_list#r'./train_list.txt'
        self.train_class_num = cfg.T_class_num#17
        self.flower17_data = []
        self.data = cfg.DATA# './PetData'
        if not os.path.isdir(self.data):#是否是目录
    ...
    def load_17Pets(self,save_name='17Pets.pkl'):
        '''
        在train_txt文件中以列为单位依次获取 图片地址、图片类别等信息
        将图片的矩阵数据(img)与图片类别数据(lable)作为一个整体进行保存
        '''
        save_path = os.path.join(self.data, save_name)#os.path.join()连接两个文件名地址的时候
        if os.path.isfile(save_path):
            self.flower17_data = pickle.load(open(save_path, 'rb'))
        else:
            with codecs.open(self.train_list, 'r', 'utf-8') as f:
                lines = f.readlines()#readlines所有数据是list，每行是一个元素
                for num, line in enumerate(lines):
                    context = line.strip().split(' ')
                    image_path = context[0]
                    index = int(context[1])#索引

                    img = cv2.imread(image_path)#(W,H,C)
                    img = resize_image(img, self.image_size, self.image_size)#224
                    print('done')
            ...
            pickle.dump(self.flower17_data,open(save_path,'wb'))#序列化对象，并将结果数据流写入到文件对象中。
    ...
    def get_batch(self):
        ...
        '''
        images = np.zeros((self.train_batch_size, self.image_size, self.image_size, 3))#64，224，224
        labels = np.zeros((self.train_batch_size, self.train_class_num))#17
        count = 0
        while( count < self.train_batch_size):
            ...
            if self.cursor >= len(self.flower17_data) :
                self.cursor = 0
                self.epoch += 1
                np.random.shuffle(self.flower17_data)#随机排序
                print(self.epoch)
...
class FineTun_And_Predict_Data(object):
    ...
    def __init__(self, solver=None, is_svm=False, is_save=True):
        ...
        self.fineturn_save_path = cfg.Fineturn_save#'./FlowerData/Fineturn'
        if not os.path.isdir(self.fineturn_save_path):
            os.makedirs(self.fineturn_save_path)

        self.SVM_and_Reg_save_path = cfg.SVM_and_Reg_save#'./FlowerData/SVM_and_Reg'
        if not os.path.isdir(self.SVM_and_Reg_save_path):
            os.makedirs(self.SVM_and_Reg_save_path)

        self.fineturn_threshold = cfg.F_fineturn_threshold#0.3
        self.svm_threshold = cfg.F_svm_threshold#0.3
        self.reg_threshold = cfg.F_regression_threshold#0.6
    ...
    def get_fineturn_batch(self):
        images = np.zeros((self.fineturn_batch_size, self.image_size, self.image_size, 3))#256
        labels = np.zeros((self.fineturn_batch_size, self.F_class_num))#3
        count = 0
    ...
    def get_Reg_batch(self):#bboxing回归
        images = np.zeros((self.Reg_batch_size, 4096))#512
        labels = np.zeros((self.Reg_batch_size, self.R_class_num))#5
        count = 0
